...
- ✅ `npx tsc --noEmit` zero errors.
## 2026-05-07 — PDF-COVER-PAGE
- **Title:** Added a professional cover page (page 1) to the issued PDF matching the NCS Q22338 reference layout; existing detail breakdown becomes page 2+
- **Status:** ✅ Resolved
- **Branch:** fix/pdf-cover-page
- **Audit reference:** Sprint goal — match NCS Q22338's two-page format. Page 1 = company-branded cover with quote summary + acceptance terms + signoff; page 2+ = the detail breakdown (already built across Sprints 1-6). Also fixes the `null.pdf` filename bug for unsaved/draft quotes.
- **Root cause / context:** The PDF previously rendered as a single page that jumped straight into the room/piece breakdown — visually unprofessional vs the NCS reference, and missed the standard "For: ..." + "Yours Sincerely" framing that customers expect. Additionally `pdf/route.ts:104` interpolated `data.quoteNumber` directly into `Content-Disposition`, producing `null.pdf` when `quote.quote_number` was null (drafts).
- ✅ **`PdfTemplateSettings` extended** (`quote-pdf-renderer.ts`): three new fields — `companyLogoUrl: string | null` (rendered on cover page when set), `signoffName: string | null` and `signoffTitle: string | null` (sourced from `companies.signature_name` / `signature_title`). `DEFAULT_TEMPLATE_SETTINGS` updated with sensible nulls. Existing fields untouched.
- ✅ **API route wiring** (`pdf/route.ts`): single `prisma.quote_templates.findFirst` became a `Promise.all([template, prisma.companies.findUnique({ select: { signature_name, signature_title } })])` — both queries fire concurrently, no extra latency. `templateSettings` now includes `companyLogoUrl: template.logo_url`, `signoffName: company?.signature_name ?? null`, `signoffTitle: company?.signature_title ?? null`.
- ✅ **Filename "null.pdf" fix** (`pdf/route.ts`): `Content-Disposition` now uses `${data.quoteNumber ?? \`Q-${quoteId}-DRAFT\`}.pdf`. Saved quotes unchanged (`Q-XXXX.pdf`); draft quotes download as `Q-156-DRAFT.pdf` instead of `null.pdf`. Captured in a `const filename` for clarity.
- ✅ **Cover-page styles** (`quote-pdf-renderer.ts`): added 22 new entries to the StyleSheet under a `// ── Cover Page ──` section header. Style language matches the existing breakdown page exactly (BLUE / DARK_GRAY / GRAY palette, Helvetica/Helvetica-Bold font family). New: `coverHeaderRow`, `coverHeaderLeft`, `coverHeaderRight`, `coverCompanyName` (16pt bold), `coverCompanyDetail`, `coverLogo` (max 120×80, contain), `coverDivider`, `coverQuoteTitleRow` + `coverQuoteTitle` (14pt bold) + `coverQuoteDate`, `coverRevision`, `coverForLine` + `coverForLabel`, `coverPara`, `coverNoteHeader` (bold underlined), `coverNoteBullet`, `coverCostBlock` + `coverCostRow` + label/value, `coverGrandTotalRow` + label/value (bold + bordered top), `coverTermsPara`, `coverSignoffBlock` + line/name/title.
- ✅ **`renderCoverPage(data, settings)` function** (`quote-pdf-renderer.ts`): returns a complete `<Page>` element. Twelve sections in spec order — (1) company header left + logo right; (2) divider; (3) quote title `Quote - {quoteNumber} - {project}` + date right-aligned; (4) revision; (5) `For: {customer.company || customer.name}` (bold label); (6) three intro paragraphs (NCS Q22338 boilerplate); (7) `PLEASE NOTE:` header + bullet (NCS boilerplate); (8) cost summary block — `Cost: $X` / `GST: $Y` / `Total Including GST: $Z` with bold + bordered final row; (9-10) three terms paragraphs (acceptance + deposit + validity, with dynamic `validityDays`); (11) signoff block — "Yours Sincerely" + name + title, falls back to `companyName` when `signoffName` is null; (12) page footer (reuses `buildPageFooter`). Uses `React.createElement` (`h` alias) per existing renderer pattern; NOT JSX.
- ✅ **Document tree updated** (`quote-pdf-renderer.ts`): the single-`<Page>` Document became a two-page Document — `renderCoverPage(data, settings)` first, then the existing breakdown `<Page>` (with new `key: 'breakdown'` for React reconciliation). The breakdown page's contents (header / quoteInfo / rooms / charges / totals / terms / footer) are completely untouched.
- ✅ **Page numbering automatic.** Both pages reuse `buildPageFooter(settings)` which uses react-pdf's `pageNumber` / `totalPages` render prop — produces "Page 1 of 2" / "Page 2 of 2" automatically. No extra wiring needed.
- **Boilerplate paragraphs hardcoded** with `// TODO: tenant-configurable via companies.quote_intro_text_*` markers above each block (intro / please-note / terms). Schema columns already exist (`companies.quote_intro_text_1/2/3`, `quote_please_note`, `quote_terms_text_1/2/3/4`) — a future sprint can thread them through `PdfTemplateSettings` without re-locating any strings.
- **Defensive fallbacks** all in place: project label degrades through `projectName → projectAddress → 'Job'`; customer org degrades through `customer.company → customer.name → 'Customer'`; quote title falls back to `Quote (Draft) - {project}` when `quoteNumber` null; signoff falls back to company name when `signoffName` null; logo simply omitted when `companyLogoUrl` null. No crashes, no blank-but-bordered slots.
- **Decisions made (worth flagging):**
  1. **Date format:** spec showed `DD/MM/YYYY` — used existing `formatAustralianDate` long form (`"7 May 2026"`) for consistency with the breakdown page's `buildQuoteInfo`. Mixing two date formats across pages would be unprofessional.
  2. **GST label on cover:** strips parenthetical from `gstLabel` ("GST (10%)") to "GST:" via `gstLabel.replace(/\s*\([^)]*\)/, '') + ':'`. NCS Q22338 cover layout has the percentage implicit; the parenthetical is appropriate on the breakdown page.
  3. **Boilerplate hardcoded** (per spec) rather than fetched from `companies.quote_intro_text_*` columns — explicit TODO markers point future sprints at the schema.
  4. **Logo rendering:** server-side `<Image src={url}>` only fires when `companyLogoUrl` is truthy. If the URL is set but unreachable at render time, react-pdf throws — that's a server-side networking concern for a separate sprint.
- **No changes to:** `pricing-calculator-v2.ts` (per spec), `pricing-rules-engine.ts`, UI calculate route, `quote-pdf-service.ts` (data assembly unchanged — cover page reads from existing fields), `TotalBreakdownAccordion.tsx`, any type files outside `PdfTemplateSettings`.
- **Verification scenario (Rule 28 / Rule 53):** after Railway deploys, download the PDF for any quote — should be 2 pages with Page 1 = cover (NCS layout), Page 2 = existing detail breakdown unchanged. Saved quotes filename = `Q-XXXX.pdf`; draft quotes filename = `Q-{id}-DRAFT.pdf`. Adjacent regression: open a pre-existing quote and confirm the breakdown page (now page 2) renders identically — no shift in totals, edges, parts, or charges.
- **Files changed:** 2 files, 323 insertions, 10 deletions, +313 net.
  - `src/lib/services/quote-pdf-renderer.ts` (+293) — `Image` import, 3 new settings fields, cover-page styles (22 entries), `renderCoverPage` function (~115 lines), Document tree split into 2 pages
  - `src/app/api/quotes/[id]/pdf/route.ts` (+20) — `Promise.all` company query, 3 new field wirings, filename fallback
- ✅ `npx tsc --noEmit` zero errors.
## 2026-05-07 — PDF-ROOM-BREAKDOWN
- **Title:** Restructured the PDF breakdown page (page 2+) to match the NCS Q22338 room-grouped description format — one description block per room, single price per room, additionals as bullets
- **Status:** ✅ Resolved
- **Branch:** fix/pdf-room-breakdown
- **Audit reference:** Sprint goal — match NCS Q22338's page-2 layout. Previous renderer produced one row per piece with per-piece price (flat list); NCS reference shows one description-style block per room with thickness-prefixed title, "Description:" label, per-piece dimension lines, bulleted additionals (waterfalls/splashbacks/cutouts), italic material name, single right-aligned room price.
- **Root cause / context:** The breakdown page rendered each piece as an independent visual block via `buildPieceRow`. Pieces in a room were grouped under a `roomHeader` bar but each carried its own price column — visually fragmented vs NCS, and didn't surface the natural "this room costs $X" framing customers expect on quotes.
- ✅ **`QuotePdfPiece` extended** (`quote-pdf-service.ts`): added `pieceType: string | null` to the type. Populated via `piece.piece_type ?? null` in the data assembly (alongside the existing `parts` field). Schema column already existed (`quote_pieces.piece_type String? @default("BENCHTOP")` per Sprint 7A's Gate 0 audit) — value range is `BENCHTOP / ISLAND / SPLASHBACK / WATERFALL / VANITY / SHELF / PANEL / OTHER`. Drives the new renderer's split between main pieces (description) and attachment pieces (bullets).
- ✅ **Renderer styles replaced** (`quote-pdf-renderer.ts`): removed the old `// ── Room Section ──` style block (`roomHeader` / `roomName` / `roomTotal` — 18 lines) and added 10 new room-block styles for the description-style layout: `roomBlock`, `roomTitleRow`, `roomTitle` (11pt bold), `roomBlockPrice` (right-aligned), `roomDescLabel` ("Description:" 9pt bold), `roomDescLine`, `roomBullet` (8pt indent), `roomMaterial` (italic), `roomBlockDivider` (0.5px rule between rooms).
- ✅ **`buildRoomSection` rewritten** to NCS description-style format:
  - Splits room pieces into main + attachments via `isMainPiece(p)` (returns false for `WATERFALL` / `SPLASHBACK`, true otherwise). Edge case: if a room has only attachments (e.g. a "Splashbacks" room with no benchtop), all pieces are treated as main so the room renders something meaningful.
  - **Room title:** `"{dominant thickness}mm {room.name}"` (e.g. `"40mm Kitchen"`). Uses `mainPieces[0].thicknessMm` as dominant thickness; falls back to bare `room.name` if no thickness.
  - **Description block:** lists every main piece's description lines under a single `Description:` label. Per-piece formatting via new `pieceDescriptionLines(p)` helper — for L/U pieces (`p.parts` populated): header `"{thickness}mm U-Shaped Benchtops"` + part lines `"1 @ {L} × {W}"`. For rectangles: single line `"L × W × Tmm Benchtop"`.
  - **Additionals bullets:** attachment pieces first via `attachmentLabel(p)` returning `"Waterfall End"` / `"Splashback"` (e.g. `"- 1 x Waterfall End"`); then cutouts across all pieces in the room (`"- {qty} x {name} cutout"`). Cutout suffix always singular (NCS Q22338 mixes; Stonehenge picks one for simplicity).
  - **Material name:** italic line — first non-null `materialName` from main pieces. Multi-material per-room is a separate gap (only first material surfaces).
  - **Compact-mode short-circuit:** when `settings.showPieceBreakdown === false`, renders only title + price + divider, preserving the previous compact behaviour.
- ✅ **`buildPieceRow` removed** — local helper, only caller was the old `buildRoomSection`. Gate 0 grep confirmed no external consumers (not exported). Net deletion: ~50 lines.
- ✅ **`buildTotals` labels updated** to NCS terminology: `"Subtotal (excl. GST)"` → `"Total Excl. GST"`, `settings.gstLabel` (was `"GST (10%)"`) → fixed `"GST"`, `"Total (incl. GST)"` → `"Total Incl. GST"`. Matches Q22338 verbatim.
- **Cover page (page 1) untouched** — `renderCoverPage` from Sprint 7A unchanged. Document tree still produces 2 pages: cover + breakdown.
- **Page numbering** unchanged — `buildPageFooter` uses react-pdf's `pageNumber` / `totalPages` render prop. Cover = Page 1, breakdown = Page 2+.
- **Decisions made (worth flagging):**
  1. **Edge profile in room title:** spec showed `"40mm Mitred Kitchen"`. I dropped the edge profile token, leaving `"40mm Kitchen"`. Reason: edge profiles in Stonehenge are per-side (`edge_top`/`edge_bottom`/`edge_left`/`edge_right` for rectangles, more for L/U), and `edgeSummary` is text like `"ARR on front and bottom, MIT on left"` — no clean single profile name to put in the title without (a) fuzzy mode-detection over edge codes and (b) a profile-name lookup at the renderer level (the edge name map is currently scoped to the data-assembly function). Both are out of this sprint's scope. Per-piece edge data is still resolved (Sprint 4) but doesn't appear in the room title.
  2. **Cutout suffix:** always appends `" cutout"` since cutout `name` values in the DB don't include the word (`"Undermount Sink"`, `"Hotplate"`, `"GPO"`, `"Tap Hole"`). Pluralisation (`"cutouts"` for qty > 1) skipped for simplicity.
  3. **Multi-material rooms:** when a room mixes materials (rare), only the first non-null `materialName` surfaces. Multi-material per-room display is a separate gap.
  4. **Old per-piece styles** (`pieceRow`, `pieceTopRow`, `pieceName`, `pieceDimensions`, `piecePrice`, `pieceDetail`) and old room-header styles (`roomHeader`, `roomName`, `roomTotal`) are now **orphan dictionary entries** in the StyleSheet — no code references them. Per Rule 7 (don't restructure beyond the task), I deliberately did NOT remove the orphan style entries. They're inert and cost nothing at runtime; a follow-up sprint can sweep them.
  5. **`PdfTemplateSettings.pricingMode` ('itemised' vs 'room_total'):** previously gated per-piece price visibility via the deleted `buildPieceRow`. The new room block always shows ONE room price (when `showRoomTotals`); `pricingMode` no longer affects piece-level rendering. Existing call sites untouched; only consumed by `buildCharges` for the `'total_only'` mode.
- **No changes to:** `pricing-calculator-v2.ts` (per spec), `pricing-rules-engine.ts`, UI calculate route, the PDF API route, `renderCoverPage` (cover page Sprint 7A intact), `TotalBreakdownAccordion.tsx`, any type files outside `QuotePdfPiece`.
- **Verification scenario for Quote 156 (Rule 28 / Rule 53):** after Railway deploys, generate the PDF. Page 1 = NCS cover (unchanged from Sprint 7A). Page 2 = U-shape Kitchen renders as `"40mm Kitchen"` title + `"$1,719.75"` right-aligned, then `Description:` → `40mm U-Shaped Benchtops` → `1 @ 2840 × 600` → `1 @ 1580 × 600` → `1 @ 1580 × 600` → `- 1 x Undermount Sink cutout` → `- 1 x Hotplate cutout` → italic `Jewel` → divider. Totals footer reads `Total Excl. GST` / `GST` / `Total Incl. GST`. Adjacent regression: any rectangle-only quote shows `"L × W × Tmm Benchtop"` per piece with no parts list and no per-piece prices.
- **Files changed:** 2 files, 185 insertions, 73 deletions, +112 net.
