...
async function main() {
  ...
  for (const edgeType of edgeTypes) {
    ...
    // rate20mm, rate40mm, baseRate excluded — admin UI controls pricing.
    // Seed only sets these on initial create, never overwrites them.
    const { isActive, rate20mm, rate40mm, baseRate, ...updateFields } = edgeType;
  ...
  // ============================================
  const clientTiers = [
    { name: 'Tier 1', description: 'Premium partners - best pricing', priority: 100, sortOrder: 1, isActive: true, material_margin_percent: 30.00 },
    { name: 'Tier 2', description: 'Regular clients - standard discounts', priority: 50, sortOrder: 2, isActive: true, material_margin_percent: 20.00 },
    { name: 'Tier 3', description: 'New clients - standard pricing', priority: 0, isDefault: true, sortOrder: 3, isActive: true, material_margin_percent: 15.00 },
  ];
