...
export function OptimizeModal({ quoteId, onClose, onSaved, defaultKerfWidth = 8 }: OptimizeModalProps) {
    ...
    const loadData = async () => {
      ...
      try {
        ...
        if (quotePieces.length === 0) {
          setError('Quote has no pieces to optimise');
          return;
        }
  ...
  const handleSaveAndClose = async () => {
    ...
    try {
      ...
      if (!response.ok) {
        const data = await response.json();
        throw new Error(data.error || 'Failed to save optimisation to quote');
      }
              ...
              <div>
                ...
                {!result && !error && (
                  <div className="flex items-center justify-center h-64 bg-gray-50 rounded-lg">
                    <p className="text-gray-500">Run optimisation to see results</p>
                  </div>
                )}
              </div>
            </div>
